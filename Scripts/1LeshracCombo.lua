...
function Main(tick)
	...
	if ScriptConfig.Hotkey and tick > sleep then
		...
		if target and GetDistance2D(target,me) <= 2000 and not target:DoesHaveModifier("modifier_item_blade_mail_reflect") and not target:DoesHaveModifier("modifier_item_lotus_orb_active") and not target:IsMagicImmune() and target:CanDie() then
			...
			if distance <= 500 and W and W:CanBeCasted() and me:CanCast()  and not target:DoesHaveModifier("modifier_leshrac_lightning_storm_slow") then
			...
			if Q and Q:CanBeCasted() and IsSlowMove(target) and me:CanCast() then
...
function Load()
	if PlayingGame() then
		local me = entityList:GetMyHero()
		if not me or me.classId ~= CDOTA_Unit_Hero_Leshrac then 
			script:Disable()
		else

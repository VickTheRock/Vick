...
function Main(tick)
	...
	if ScriptConfig.Hotkey and tick > sleep then
		...
		if target and GetDistance2D(target,me) <= 2000 and not target:DoesHaveModifier("modifier_item_blade_mail_reflect") and not target:DoesHaveModifier("modifier_item_lotus_orb_active") and not target:IsMagicImmune() and target:CanDie() then
			...
			if W and W:CanBeCasted() and me:CanCast() and target:DoesHaveModifier("modifier_slardar_amplify_damage") and distance < attackRange+90 and not W.abilityPhase and not inv then
				table.insert(castQueue,{1000+math.ceil(W:FindCastPoint()*1000),W})        
			end
			if W and W:CanBeCasted() and me:CanCast() and distance < attackRange+90 and not W.abilityPhase and not inv then
				table.insert(castQueue,{1000+math.ceil(W:FindCastPoint()*1000),W})        
			end
			if Q and Q:CanBeCasted() and me:CanCast()  and not inv then
			...
			if me.mana < me.maxMana*0.5 and ScriptConfig.Arcan and arcane and arcane:CanBeCasted()  and not inv then
			...
			if me.mana < me.maxMana*0.5 and ScriptConfig.Soul and soulring and soulring:CanBeCasted()  and not inv then
				table.insert(castQueue,{100,soulring})
			end
			if R and R:CanBeCasted() and me:CanCast()  and not inv and not target:DoesHaveModifier("modifier_slardar_amplify_damage") or linkens then 
				me:CastAbility(R,target)
			end
			if dagon and dagon:CanBeCasted() and me:CanCast()  and not inv and linkens then
				table.insert(castQueue,{1000+math.ceil(dagon:FindCastPoint()*1000),dagon,target})
			end
			if urn and urn:CanBeCasted() and me:CanCast() and not inv and linkens then
				table.insert(castQueue,{math.ceil(urn:FindCastPoint()*1000),urn,target})
			end	
			if halberd and halberd:CanBeCasted() and me:CanCast() and not inv and linkens then
				table.insert(castQueue,{1000+math.ceil(halberd:FindCastPoint()*1000),halberd,target})
			end
			if not slow then
				me:Attack(target)
...
function Load()
	if PlayingGame() then
		local me = entityList:GetMyHero()
		if not me or me.classId ~= CDOTA_Unit_Hero_Slardar then 
			script:Disable()
		else

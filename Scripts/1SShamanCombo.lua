...
function Main(tick)
	...
	if ScriptConfig.Hotkey and tick > sleep then
		...
		if target and GetDistance2D(target,me) <= 2000 and not target:DoesHaveModifier("modifier_item_lotus_orb_active") and not target:IsMagicImmune() and target:CanDie() then
			...
			local wards = entityList:FindEntities({classId=CDOTA_BaseNPC_ShadowShaman_Serpentward,controllable=true,alive=true,visible=true})
			...
			local attackRange = me.attackRange	
			local RangeBlink = 1700
			if (ScriptConfig.Blink) and GetDistance2D(me,target) <= RangeBlink and blink and blink:CanBeCasted() and me:CanCast() and distance > attackRange+200 and not blink.abilityPhase then
			...
			if E and E:CanBeCasted() and me:CanCast() and not target:DoesHaveModifier("modifier_shadow_shaman_voodoo") --[[and SleepCheck("allcast")]] then  
			...
			if sheep and sheep:CanBeCasted() and me:CanCast() and not target:DoesHaveModifier("modifier_shadow_shaman_voodoo") then
			...
			if W and W:CanBeCasted() and me:CanCast() and not target:DoesHaveModifier("modifier_shadow_shaman_shackles") and not target:IsHexed() then
				table.insert(castQueue,{math.ceil(W:FindCastPoint()*1000),W,target})
			end
			if R and R:CanBeCasted() and me:CanCast() and target:DoesHaveModifier("modifier_shadow_shaman_voodoo") or target:DoesHaveModifier("modifier_shadow_shaman_shackles") and SleepCheck("need") then
				local CP = R:FindCastPoint()
				local speed =1200  
				local distance = GetDistance2D(target, me)
				local delay =50+client.latency
				local xyz = SkillShot.SkillShotXYZ(me,target,delay,speed)
					if xyz and distance <= 1200  then  
						me:SafeCastAbility(R, xyz)
						Sleep(client.latency,"need")
					end
				end
...
function Load()
	if PlayingGame() then
		local me = entityList:GetMyHero()
		if not me or me.classId ~= CDOTA_Unit_Hero_ShadowShaman then 
			script:Disable()
		else

...
ScriptConfig:SetName("ChaosKnight")
ScriptConfig:SetExtention(-.3)
ScriptConfig:SetVisible(false)
--ScriptConfig:AddParam("MinimumHP","MinimumHP",SGC_TYPE_NUMCYCLE,false,250,nil,50,400,50)
...
function Main(tick)
	...
	if ScriptConfig.Hotkey and tick > sleep then
		...
		if target and GetDistance2D(target,me) <= 2500  and not target:IsMagicImmune() then
			...
			if me.mana < me.maxMana*0.5 and  soulring and soulring:CanBeCasted() and distance <= attackRange+300 then
			...
			if (ScriptConfig.Ult) and R and R:CanBeCasted() and me:CanCast() and me:DoesHaveModifier("modifier_chaos_knight_reality_rift_bonus_damage") and actv then 
					table.insert(castQueue,{1000+math.ceil(R:FindCastPoint()*1000),R})
			end
			if (ScriptConfig.Ult) and R and R:CanBeCasted() and me:CanCast() and me:DoesHaveModifier("modifier_chaos_knight_reality_rift_bonus_damage") and not armlet then 
					table.insert(castQueue,{1000+math.ceil(R:FindCastPoint()*1000),R})
			end
...
function Load()
	if PlayingGame() then
		local me = entityList:GetMyHero()
		if not me or me.classId ~= CDOTA_Unit_Hero_ChaosKnight then 
			script:Disable()
		else
